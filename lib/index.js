...
var GTM = module.exports = integration('Google Tag Manager')
  ...
  .tag('no-env-nonce', '<script nonce="{{ nonce }}" src="//www.googletagmanager.com/gtm.js?id={{ containerId }}&l=dataLayer">')
  .tag('with-env-nonce', '<script nonce="{{ nonce }}" src="//www.googletagmanager.com/gtm.js?id={{ containerId }}&l=dataLayer&gtm_preview={{ environment }}">');
...
GTM.prototype.initialize = function() {
  ...
  let suffix = "";
  if (this.options.nonce.length) {
    suffix += "-nonce";
  }
  if (this.options.environment.length) {
    this.load('with-env' + suffix, this.options, this.ready);
  } else {
    this.load('no-env' + suffix, this.options, this.ready);
  }
};
